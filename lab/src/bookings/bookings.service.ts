...
export class BookingsService {
  ...
  async uploadScan(bookingId: string, file: Express.Multer.File, doctorName?: string) {
  ...
  async uploadResult(
    bookingId: string,
    body: { title: string; resultData: string; doctorName?: string },
  ) {
    ...
    const uploaded = await new Promise<any>((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        { folder: 'medical_records/results', resource_type: 'raw' },
        (error, result) => {
